...
import { loginIdToInternalEmail, signupSchema } from '@/lib/auth/credentials';
...
export async function POST(request: NextRequest) {
  ...
  const { displayName, loginId, pin, role } = parsed.data;
  try {
    if (hasSupabaseAuthEnvironment()) {
      ...
      const { data, error } = await admin.auth.admin.createUser({
        ...
        password: pin,
        email_confirm: true,
        app_metadata: { role, demo_account: true, auth_method: 'login_id' },
      ...
      try {
        ...
        const { error: loginError } = await client.auth.signInWithPassword({ email: loginIdToInternalEmail(loginId), password: pin });
    ...
    if (raw.includes('이미 사용') || /already.*registered/i.test(raw)) {
      return NextResponse.json({ error: '이미 가입된 아이디예요. 다른 아이디를 사용해 주세요.' }, { status: 409 });
    }
