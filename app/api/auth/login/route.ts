...
import { loginIdToInternalEmail, loginSchema } from '@/lib/auth/credentials';
...
export async function POST(request: NextRequest) {
  ...
  if (hasSupabaseAuthEnvironment()) {
    ...
    const { data, error } = await client.auth.signInWithPassword({ email: loginIdToInternalEmail(loginId), password: pin });
    const role = data.user?.app_metadata.role;
    if (error || !data.user || !['senior', 'family', 'worker'].includes(role)) {
      return NextResponse.json({ error: '아이디 또는 비밀번호를 확인해 주세요.' }, { status: 401 });
    }
    return NextResponse.json({ redirectTo: `/${role}` }, { headers: response.headers });
  }

  const account = await authenticateDemoAccount(loginId, pin);
  if (!account) return NextResponse.json({ error: '아이디 또는 비밀번호를 확인해 주세요.' }, { status: 401 });
