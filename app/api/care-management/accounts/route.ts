...
import { virtualPhoneSchema } from '@/lib/auth/credentials';
...
export async function GET(request: NextRequest) {
  ...
  const parsed = virtualPhoneSchema.safeParse(request.nextUrl.searchParams.get('phone') ?? '');
  if (!parsed.success) return NextResponse.json({ error: parsed.error.issues[0]?.message }, { status: 400 });
  const profile = await careRelationships.findProfileByPhone(parsed.data);
