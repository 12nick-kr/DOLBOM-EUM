...
import { demoSeniorId, id } from '@/lib/server/store';
export async function POST(request: NextRequest) {
  ...
  return NextResponse.json({ id: documentId, object_path: secureObjectPath(demoSeniorId, documentId, ext), review_status: 'uploaded', is_demo: true, notice: '합성 데모 문서만 허용되며 원본은 외부 AI로 전송하지 않아요.' }, { status: 201 });
}
