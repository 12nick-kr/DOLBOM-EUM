...
// DB 원문 에러(테이블/컬럼/제약조건명 등 스키마 정보 포함)를 클라이언트에 그대로 노출하지 않는다. 서버 로그에는 원인을 남기고, 호출자에게는 고정 메시지만 전달한다.
function dbError(error: { message: string }): Error {
  console.error('[SupabaseCareRelationshipRepository]', error.message);
...
export class SupabaseCareRelationshipRepository implements CareRelationshipRepository {
  ...
  async link(input: { actorId: string; seniorId: string; memberId: string; relationshipType: 'family' | 'worker' }) {
    ...
    if (groupError) throw dbError(groupError);
