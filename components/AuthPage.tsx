...
export function AuthPage({ mode }: { mode: 'login' | 'signup' }) {
  ...
  async function submit(event: FormEvent) {
    ...
    setSubmitting(true);
    setError('');
    const body = mode === 'signup' ? { displayName, loginId, pin, pinConfirm, role } : { loginId, pin };
    try {
  ...
  return <main className="auth-shell">
    <section className="auth-card" aria-labelledby="auth-title">
      ...
      <form className="auth-form" onSubmit={submit}>
        ...
        <label><span>전화번호형 아이디</span><input inputMode="numeric" autoComplete="username" value={loginId} onChange={(event) => setLoginId(formatLoginId(event.target.value))} maxLength={13} required placeholder={mode === 'signup' ? '예: 010-0000-1234' : '010-0000-0001'} /></label>
