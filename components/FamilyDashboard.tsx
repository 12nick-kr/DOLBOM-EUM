  ...
  return <main className="family-shell"><DemoBadge />
    ...
    {view === 'home' && <>
      ...
      <section className="section-block care-card-feed" aria-label="최근 돌봄 요청">
        ...
        {isLoading ? <p className="notice" role="status">요청을 불러오는 중이에요.</p> : requests.length === 0 ? <p className="notice">공유된 요청이 없어요.</p> : requests.map((item) => <CareRequestCard key={item.id} card={item} role="family" />)}
      </section>
    </>}
