...
type StatFilter = 'emergency' | 'new' | 'in_progress' | 'done';
const statFilterLabel: Record<StatFilter, string> = { emergency: '미확인 긴급 알림', new: '새 요청', in_progress: '처리 중 요청', done: '완료 요청' };
...
export function WorkerDashboard() {
  ...
  const primarySeniorName = requests.find((item) => item.seniorName)?.seniorName ?? '담당';
  ...
  const openRequest = (id: string) => { setSelectedId(id); acknowledge(id); setIsEditingMemo(false); setPage('request'); };
  ...
  const statusActions = (request: ServiceRequest) => (
    <div className="status-actions">
      <button className="status-action" disabled={request.status !== 'new'} onClick={() => takeCharge(request.id)}>진행중</button>
      <button className="status-action" disabled={request.status !== 'in_progress'} onClick={() => completeRequest(request.id)}>완료</button>
    </div>
  );
  ...
  return <main className="worker-layout">
    ...
    {statModalFilter && <div className="modal-backdrop" role="presentation" onClick={() => setStatModalFilter(null)}><section className="confirm-dialog stat-modal" role="dialog" aria-modal="true" aria-labelledby="stat-modal-title" onClick={(event) => event.stopPropagation()}><div className="stat-modal-header"><h2 id="stat-modal-title">{statFilterLabel[statModalFilter]}</h2><button className="stat-modal-close" aria-label="닫기" onClick={() => setStatModalFilter(null)}>✕</button></div><div className="care-card-feed">
      {statModalFilter === 'emergency'
        ...
        : (requests.filter((request) => request.status === statModalFilter).length === 0
          ? <p className="notice">해당하는 요청이 없어요.</p>
          : requests.filter((request) => request.status === statModalFilter).map((item) => <CareRequestCard card={item} role="worker" key={item.id} onSelect={() => { setStatModalFilter(null); openRequest(item.id); }} actions={statusActions(item)} />))}
    </div></section></div>}
