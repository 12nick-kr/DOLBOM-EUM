...
export function WorkerDashboard() {
  ...
  return <main className="worker-layout">
    ...
    <section className="worker-content"><DemoBadge />
      ...
      {page === 'request' && selected && <><button className="back" onClick={() => setPage('inbox')}>‹ 요청 업무함</button><header className="worker-header"><div><StatusPill status={statusLabelFor('worker', selected.status)} /><h1>{requestTypeLabel[selected.type]}</h1></div></header><CareRequestCard card={selected} role="worker" onDelete={askDelete} deleting={deletingId === selected.id} /><section className="request-detail"><div className="card"><p className="ai-pill">🤖 AI 신청 초안</p><h2>노인맞춤돌봄서비스 연계 검토</h2><p>대상 조건은 담당자가 확인해야 해요.</p></div><label className="memo"><span>담당자 메모</span><textarea placeholder="확인한 사실만 기록해 주세요." /></label></section><div className="case-actions"><button>정보 확인 요청</button>{selected.status === 'new' && <button className="primary" onClick={() => takeCharge(selected.id)}>담당 맡기</button>}{selected.status === 'in_progress' && <button className="primary" onClick={() => completeRequest(selected.id)}>처리 완료</button>}</div></>}
