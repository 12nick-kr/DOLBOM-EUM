...
export function CareConnectionManager() {
  ...
  const search = async () => {
    setLoading(true);
    setMessage('');
    const response = await fetch(`/api/care-management/accounts?phone=${encodeURIComponent(phone)}`);
    const body = await response.json();
    setProfile(response.ok ? body.profile : null);
    setMessage(response.ok ? body.profile ? '' : '해당 가상 번호의 계정이 없어요.' : body.error ?? '검색하지 못했어요.');
    setLoading(false);
  };
  ...
  const link = async () => {
    ...
    if (response.ok) { setProfile(null); setPhone(''); await loadGroups(); }
    setLoading(false);
  };
