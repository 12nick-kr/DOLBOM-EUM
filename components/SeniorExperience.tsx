...
export function SeniorExperience() {
  ...
  return <main className="senior-shell" data-density="comfort">
    <DemoBadge />
    <header className="senior-header"><span>돌봄이음 AI</span><div className="session-actions"><strong>{clock}</strong><LogoutButton className="header-logout" /></div></header>
    {screen.view === 'home' && <section className="senior-home"><p>{profile ? `${profile.displayName}님,` : '안녕하세요,'}</p><h1>무엇을 도와드릴까요?</h1><button className="talk-button" onClick={() => void startRecording('request')} aria-label="말하기 시작">🎙️<small>눌러서 말하기</small></button><label className="sr-only" htmlFor="senior-text">도움 요청 입력</label><div className="text-entry"><input id="senior-text" value={input} onChange={(event) => setInput(event.target.value)} onKeyDown={(event) => { if (event.key === 'Enter') submitTextInput(); }} /><button onClick={submitTextInput}>보내기</button></div><div className="two-actions"><button onClick={() => dispatch({ type: 'REQUESTS' })}>📋 내 요청 보기</button><button onClick={() => dispatch({ type: 'INFO' })}>💙 복지 정보</button></div></section>}
