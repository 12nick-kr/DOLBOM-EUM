...
describe('risk and permission policies', () => {
  ...
  it('allows only defined request state transitions and confirmation tokens', () => { expect(canTransitionRequest('new', 'connecting')).toBe(true); expect(canTransitionRequest('completed', 'new')).toBe(false); expect(needsConfirmation()).toBe(true); expect(needsConfirmation('confirmed')).toBe(false); });
});
