...
describe('POST /api/ai/speech requires ownership of the assistant turn', () => {
  it('returns 403 when the requesting senior does not own the assistant_turn_id', async () => {
    ...
    const respondReq = new NextRequest('http://localhost:3000/api/ai/respond', { method: 'POST', body: JSON.stringify({ text: '안녕하세요', seniorId: 'senior-owner' }), headers: { 'Content-Type': 'application/json' } });
    ...
    const speechReq = new NextRequest('http://localhost:3000/api/ai/speech', { method: 'POST', body: JSON.stringify({ assistant_turn_id: turn.id, senior_id: 'someone-else' }), headers: { 'Content-Type': 'application/json' } });
